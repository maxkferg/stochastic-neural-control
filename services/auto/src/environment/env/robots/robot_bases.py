...
class BaseRobot:
    ...
    """
    def __init__(self, model_file, robot_name, physics, scale=1, env=None):
        self.parts = None
        self.jdict = None
    ...
    def addToScene(self, bodies):
        ...
        for i in range(len(bodies)):
            if self.physics.getNumJoints(bodies[i]) == 0:
                ...
                parts[part_name] = BodyPart(part_name, bodies, i, -1, self.scale, self.physics, model_type=self.model_type)
            ...
            for j in range(p.getNumJoints(bodies[i])):
                ...
                parts[part_name] = BodyPart(part_name, bodies, i, j, self.scale, self.physics, model_type=self.model_type)

                if part_name == self.robot_name:
                    self.robot_body = parts[part_name]

                if i == 0 and j == 0 and self.robot_body is None:  # if nothing else works, we take this as robot_body
                    parts[self.robot_name] = BodyPart(self.robot_name, bodies, 0, -1, self.scale, self.physics, model_type=self.model_type)
                    self.robot_body = parts[self.robot_name]

                if joint_name[:6] == "ignore":
                    Joint(joint_name, bodies, i, j, self.scale, self.physics).disable_motor()
                    continue
    ...
    def _load_model(self):
        if self.model_type == "MJCF":
            self.robot_ids = self.physics.loadMJCF(os.path.join(self.physics_model_dir, self.model_file), flags=p.URDF_USE_SELF_COLLISION+p.URDF_USE_SELF_COLLISION_EXCLUDE_ALL_PARENTS)
...
class BodyPart:
    def __init__(self, body_name, bodies, bodyIndex, bodyPartIndex, scale, physics, model_type):
        self.bodies = bodies
        self.physics = physics
...
class Joint:
    def __init__(self, joint_name, bodies, bodyIndex, jointIndex, scale, model_type, physics):
        ...
        _,_,self.jointType,_,_,_,_,_,self.lowerLimit, self.upperLimit,_,_,_, _,_,_,_ = self.physics.getJointInfo(self.bodies[self.bodyIndex], self.jointIndex)
        self.power_coeff = 0
        if model_type=="mjcf":
            self.scale = scale
        else:
