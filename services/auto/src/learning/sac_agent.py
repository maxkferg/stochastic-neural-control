...
class SacAgent(BaseAgent):
    ...
    def __init__(
        ...
        # State encoders. Policy uses q1_encoder
        self.q1_encoder = StateEncoderCls()
        self.q2_encoder = StateEncoderCls()
        self.target_q1_encoder = StateEncoderCls()
        self.target_q2_encoder = StateEncoderCls()
        model_kwargs["state_encoder"] = self.q1_encoder
        # Initialize base
    ...
    def initialize(self, env_spaces, share_memory=False,
        ...
        # Create models
        self.q1_model = self.QModelCls(state_encoder=self.q1_encoder, **self.env_model_kwargs, **self.q_model_kwargs)
        self.q2_model = self.QModelCls(state_encoder=self.q2_encoder, **self.env_model_kwargs, **self.q_model_kwargs)
        self.target_q1_model = self.QModelCls(state_encoder=self.target_q1_encoder,
            **self.env_model_kwargs, **self.q_model_kwargs)
        self.target_q2_model = self.QModelCls(state_encoder=self.target_q2_encoder,
            **self.env_model_kwargs, **self.q_model_kwargs)
    ...
    def print_parameter_info(self):
        ...
        print("\nQ1 Parameters:")
        for name, param in self.q1_model.named_parameters():
            if param.requires_grad:
                print(name, tuple(param.shape))
        print("\nQ2 Parameters:")
        for name, param in self.q2_model.named_parameters():
            if param.requires_grad:
                print(name, tuple(param.shape))
