...
class Roadmap():
    ...
    def __init__(self, ox, oy, rr, show_animation=False):
        self.obkdtree = KDTree(np.vstack((ox, oy)).T)
        sample_x, sample_y = sample_points(rr, ox, oy, self.obkdtree)
        if show_animation:
        ...
        self.road_map = generate_roadmap(sample_x, sample_y, self.rr, self.obkdtree)
        self.skdtree = KDTree(np.vstack((sample_x, sample_y)).T)


    def connect_to_graph(self, px, py):
        """
        Connect a point to the sample graph
        Return the sx,sy from the sampled point OR None for failure
        """
        index, dist = self.skdtree.search(np.array([px, py]).reshape(2, 1), 3)
        for i in index[0]:
            sx, sy = self.sample_x[i], self.sample_y[i] 
            if not is_collision(sx, sy, px, py, self.rr, self.obkdtree):
                return sx, sy
        return None, None


    def solve(self, sx, sy, gx, gy):
        """
        Return the path from the start to the goal
        The start and the goal are not included
        """
        ox = self.ox.copy()
        oy = self.oy.copy()
        rr = self.rr
        ...
        start_x, start_y = self.connect_to_graph(sx, sy)
        goal_x, goal_y = self.connect_to_graph(gx, gy)
        
        if start_x is None or goal_x is None:
            return [],[]

        print("NODES:",start_x,start_y,goal_x,goal_y)
    ...
    def render(self, sx, sy, gx, gy):
        """
        Render the motion planning problem
        """
        import plotly.graph_objects as go
        shapes = []
        for center in zip(self.ox, self.oy):
            shapes.append(
                go.layout.Shape(
                    type="circle",
                    xref="x",
        ...
        # Connected start nodes
        sxx, syy = self.connect_to_graph(sx, sy)
        shapes.append(
            go.layout.Shape(
                type="circle",
                xref="x",
        ...
        gxx, gyy = self.connect_to_graph(gx, gy)
        shapes.append(
            go.layout.Shape(
                type="circle",
                xref="x",
...
def PRM_planning(sx, sy, gx, gy, ox, oy, rr):
    ...
    sample_x, sample_y = sample_points(rr, ox, oy, obkdtree)
    if show_animation:
        plt.plot(sample_x, sample_y, ".b")
...
def sample_points(rr, ox, oy, obkdtree):
    maxx = max(ox)
    maxy = max(oy)
    minx = min(ox)
    miny = min(oy)
