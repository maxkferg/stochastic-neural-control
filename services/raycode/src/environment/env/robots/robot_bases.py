...
class BaseRobot:
    ...
    """
    def __init__(self, model_file, robot_name, scale = 1, physics=None, env = None):
        self.parts = None
        self.jdict = None
    ...
    def addToScene(self, bodies):
        ...
        for i in range(len(bodies)):
            if self.physics.getNumJoints(bodies[i]) == 0:
                part_name, robot_name = self.physics.getBodyInfo(bodies[i], 0)
                robot_name = robot_name.decode("utf8")
                part_name = part_name.decode("utf8")
                parts[part_name] = BodyPart(part_name, bodies, i, -1, self.scale, model_type=self.model_type, physics=self.physics)

            for j in range(self.physics.getNumJoints(bodies[i])):
                self.physics.setJointMotorControl2(bodies[i],j,p.POSITION_CONTROL,positionGain=0.1,velocityGain=0.1,force=0)
                ## TODO (hzyjerry): the following is diabled due to pybullet update
                #_,joint_name,joint_type, _,_,_, _,_,_,_, _,_, part_name = self.physics.getJointInfo(bodies[i], j)
                _,joint_name,joint_type, _,_,_, _,_,_,_, _,_, part_name, _,_,_,_ = self.physics.getJointInfo(bodies[i], j)
                ...
                parts[part_name] = BodyPart(part_name, bodies, i, j, self.scale, model_type=self.model_type, physics=self.physics)

                if part_name == self.robot_name:
                    self.robot_body = parts[part_name]

                if i == 0 and j == 0 and self.robot_body is None:  # if nothing else works, we take this as robot_body
                    parts[self.robot_name] = BodyPart(self.robot_name, bodies, 0, -1, self.scale, model_type=self.model_type, physics=self.physics)
                    self.robot_body = parts[self.robot_name]

                if joint_name[:6] == "ignore":
                    Joint(joint_name, bodies, i, j, self.scale, physics=self.physics).disable_motor()
                    continue

                if joint_name[:8] != "jointfix" and joint_type != p.JOINT_FIXED:
                    joints[joint_name] = Joint(joint_name, bodies, i, j, self.scale, model_type=self.model_type, physics=self.physics)
    ...
    def _load_model(self):
        if self.model_type == "MJCF":
            self.robot_ids = self.physics.loadMJCF(os.path.join(self.physics_model_dir, self.model_file), flags=self.physics.URDF_USE_SELF_COLLISION+self.physics.URDF_USE_SELF_COLLISION_EXCLUDE_ALL_PARENTS)
        if self.model_type == "URDF":
            self.robot_ids = (self.physics.loadURDF(os.path.join(self.physics_model_dir, self.model_file), globalScaling = self.scale), )
...
class Pose_Helper:
    ...
    def rpy(self):
        return self.physics.getEulerFromQuaternion(self.body_part.get_orientation())
...
class BodyPart:
    def __init__(self, body_name, bodies, bodyIndex, bodyPartIndex, scale, model_type, physics):
        self.physics = physics
        self.bodies = bodies
    ...
    def _set_fields_of_pose_of(self, pos, orn):
        ...
        self.physics.resetBasePositionAndOrientation(self.bodies[self.bodyIndex], np.array(pos) / self.scale, orn)
    ...
    def speed(self):
        if self.bodyPartIndex == -1:
            (vx, vy, vz), _ = self.physics.getBaseVelocity(self.bodies[self.bodyIndex])
        else:
            (x,y,z), (a,b,c,d), _,_,_,_, (vx, vy, vz), (vr,vp,vyaw) = self.physics.getLinkState(self.bodies[self.bodyIndex], self.bodyPartIndex, computeLinkVelocity=1)
        return np.array([vx, vy, vz])

    def angular_speed(self):
        if self.bodyPartIndex == -1:
            _, (vr,vp,vyaw) = self.physics.getBaseVelocity(self.bodies[self.bodyIndex])
        else:
            (x,y,z), (a,b,c,d), _,_,_,_, (vx, vy, vz), (vr,vp,vyaw) = self.physics.getLinkState(self.bodies[self.bodyIndex], self.bodyPartIndex, computeLinkVelocity=1)
        return np.array([vr, vp, vyaw])

    def contact_list(self):
        return self.physics.getContactPoints(self.bodies[self.bodyIndex], -1, self.bodyPartIndex, -1)


class Joint:
    def __init__(self, joint_name, bodies, bodyIndex, jointIndex, scale, model_type, physics):
        self.physics = physics
        self.bodies = bodies
        self.bodyIndex = bodyIndex
        ...
        if model_type=="mjcf":
            ...
            self.scale = 1
        if self.jointType == self.physics.JOINT_PRISMATIC:
    ...
    def get_state(self):
        """Get state of joint
           Position is defined in real world scale """
        x, vx,_,_ = self.physics.getJointState(self.bodies[self.bodyIndex],self.jointIndex)
        if self.jointType == self.physics.JOINT_PRISMATIC:
            x  *= self.scale
            vx *= self.scale
        return x, vx
    ...
    def set_state(self, x, vx):
        ...
        if self.jointType == self.physics.JOINT_PRISMATIC:
            x  /= self.scale
            vx /= self.scale
        self.physics.resetJointState(self.bodies[self.bodyIndex], self.jointIndex, x, vx)
    ...
    def set_position(self, position):
        ...
        self.physics.setJointMotorControl2(self.bodies[self.bodyIndex],self.jointIndex,p.POSITION_CONTROL, targetPosition=position)
    ...
    def set_velocity(self, velocity):
        ...
        self.physics.setJointMotorControl2(self.bodies[self.bodyIndex],self.jointIndex,p.VELOCITY_CONTROL, targetVelocity=velocity) # , positionGain=0.1, velocityGain=0.1)

    def set_torque(self, torque):
        self.physics.setJointMotorControl2(bodyIndex=self.bodies[self.bodyIndex], jointIndex=self.jointIndex, controlMode=p.TORQUE_CONTROL, force=torque) #, positionGain=0.1, velocityGain=0.1)

    def reset_state(self, pos, vel):
        self.set_state(pos, vel)

    def disable_motor(self):
        self.physics.setJointMotorControl2(self.bodies[self.bodyIndex],self.jointIndex,controlMode=p.POSITION_CONTROL, targetPosition=0, targetVelocity=0, positionGain=0.1, velocityGain=0.1, force=0)
